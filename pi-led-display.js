...
class PiLedDisplay {
    constructor(config) {
        ...
        this.outputMethod =
            this.config.output === 'console'
                ? this.renderCanvasToConsole
                : this.renderCanvasToMatrix;

        // canvas that will be output to led matrix or terminal
        this.canvasWidth =
            this.config.ledMatrix.columns *
            this.config.ledMatrix.parallelDisplays;
        this.canvasHeight =
            this.config.ledMatrix.rows * this.config.ledMatrix.chainedDisplays;
        ...
        this.animationInterval = Math.floor(
            1000 / this.config.ledMatrix.frameRate
        );
    }
    ...
    getMergedLayerObservable() {
        ...
        if (this.sceneLayers.length > 1) {
            return this.sceneLayers.reduce((merged, source) => {
                return merge(
                    merged.getCompleteObservable(),
                    source.getCompleteObservable()
                );
            });
        } else {
    ...
    nextScene() {
        this.clearScene();
        this.sceneInt =
            this.sceneInt + 1 >= this.scenes.length ? 0 : this.sceneInt + 1;
    ...
    initLedMatrix() {
        this.matrix = new LedMatrix(
            this.config.ledMatrix.rows,
            this.config.ledMatrix.columns,
            this.config.ledMatrix.chainedDisplays,
            this.config.ledMatrix.parallelDisplays,
            this.config.ledMatrix.brightness,
            this.config.ledMatrix.hardwareMapping
        );
    }
    ...
    renderCanvasToMatrix() {
        return Observable.create((observer) => {
            try {
                ...
                observer.error(
                    `There was an error rendering canvas to console. ${err}`
                );
            }
        });
    }
    ...
    renderLayers() {
        return Observable.create((observer) => {
            try {
                const renderedLayers = this.sceneLayers.map((layer) =>
                    layer.getFrame()
                );
    ...
    layersToCanvas(renderedLayers) {
        return Observable.create((observer) => {
            try {
                ...
                observer.error(
                    `There was an error applying layers to canvas. ${err}`
                );
            }
        });
    }
    ...
    animateFrame() {
        return Observable.create((observer) => {
            this.renderLayers()
                .pipe(
                    flatMap((renderedLayers) =>
                        this.layersToCanvas(renderedLayers)
                    ),
                ...
                .subscribe(
                    ...
                    (err) => {
                        observer.error(
                            `There was an error rendering the frame. ${err}`
                        );
                    }
                );
        });
    ...
    animationLoop() {
        ...
        clock.subscribe((state) => {
            if (rendering === false) {
                ...
                this.animateFrame().subscribe(
                    () => {
                        rendering = false;
                    },
                    (err) => {
                        console.error(
                            'There was an error rendering the frame.',
                            err
                        );
                    }
                );
